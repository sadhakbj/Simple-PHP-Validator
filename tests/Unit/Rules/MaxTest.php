...
it('fails when the string length exceeds the limit', function () {
    $rule = new Max(5);
    expect($rule->passes('field', 'abcdef'))->toBeFalse();
});

it('passes when the array length is within the limit', function () {
    $rule = new Max(5);
    expect($rule->passes('field', range(1, 3)))->toBeTrue()->and($rule->passes('field', range(1, 5)));
});

it('passes when the array length is greater the limit', function () {
    $rule = new Max(5);
    expect($rule->passes('field', range(1, 6)))->toBeFalse();
});
